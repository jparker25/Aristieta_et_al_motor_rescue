...
direcs = ["jaws_neurons/pre_opto","jaws_neurons/post_opto","npas_neurons/pre_opto","npas_neurons/post_opto"];

for direc = 1:length(direcs)
    spike_files = dir(fullfile(sprintf('../data/training/%s/spikes/*.txt',direcs(direc))));
    recording_lengths = importdata(sprintf('../data/training/%s/cell_lengths.txt',direcs(direc)));
    firing_rates = importdata(sprintf('../data/training/%s/cell_frs.txt',direcs(direc)));
